...
@pytest.fixture
def jwt_token(crear_usuario, crear_roles):
    client = APIClient()
...
def test_obtener_permisos(api_client, jwt_token):
...
class TestPlanViewSet:
    def test_list_planes(self, api_client, plan):
    ...
    def test_create_plan(self, api_client, plan_data):
        url = reverse("plan-list")
        response = api_client.post(url, plan_data, format="json")
        assert response.status_code == status.HTTP_201_CREATED
        assert Plan.objects.count() == 1

    def test_retrieve_plan(self, api_client, plan_data):
    ...
    def test_update_plan(self, api_client, plan_data):
        plan = Plan.objects.create(**plan_data)
        url = reverse("plan-detail", args=[plan.id])
    ...
    def test_delete_plan(self, api_client, plan_data):
        plan = Plan.objects.create(**plan_data)
        url = reverse("plan-detail", args=[plan.id])
